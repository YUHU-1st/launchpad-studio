...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String intentAddress = getIntent().getStringExtra(EXTRA_ADDRESS);
        String intentPin = getIntent().getStringExtra(EXTRA_PIN);
        String savedAddress = prefs.getString("address", "");
        String savedPin = prefs.getString("pin", "");
        String address = intentAddress == null || intentAddress.trim().isEmpty() ? savedAddress : intentAddress;
        String pin = intentPin == null || intentPin.trim().isEmpty() ? savedPin : intentPin;
        addressInput.setText(address);
        pinInput.setText(pin);
        if (!address.isEmpty() && pin.matches("\\d{6}")) connect(address, pin);
    }
    ...
    private View buildUi() {
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
                if (request.isForMainFrame()) {
                    showConnectionPanel("无法连接电脑，请检查地址、Wi-Fi 和桌面遥控服务。");
                }
            }
            ...
            public void onReceivedHttpError(WebView view, WebResourceRequest request,
                ...
                if (request.isForMainFrame() && response.getStatusCode() >= 400) {
                    showConnectionPanel("电脑拒绝连接，请检查地址和配对 PIN。");
                }
            }
        });
        ...
        content.addView(connectionPanel, new FrameLayout.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
    ...
    private LinearLayout buildConnectionPanel() {
        ...
        card.addView(text("连接电脑", 23, TEXT));
        TextView help = text("在电脑端 Launchpad Studio 点击“手机遥控”，把局域网地址和 6 位 PIN 填到这里。", 14, MUTED);
        help.setPadding(0, dp(8), 0, dp(16));
        card.addView(help);

        addressInput = edit("电脑地址，例如 http://192.168.1.10:8765",
        ...
        cardParams.width = Math.min(dp(560), getResources().getDisplayMetrics().widthPixels - dp(44));
    ...
    private void connect(String rawAddress, String rawPin) {
        ...
        if (address.isEmpty()) {
            showConnectionPanel("请输入电脑地址。");
            return;
        }
        if (!pin.matches("\\d{6}")) {
            showConnectionPanel("配对 PIN 应为 6 位数字。");
            return;
        }
        ...
        // PIN is exactly six ASCII digits. Concatenating it is API-29-safe and
        // avoids URLEncoder.encode(String, Charset), which is unavailable on the
        // DJI RC Plus Android 10 runtime.
        webView.loadUrl(address + "/?token=" + pin);
    }
    ...
    private EditText edit(String hint, int inputType) {
        ...
        input.setBackgroundColor(Color.rgb(27, 33, 48));
        return input;
    }
    ...
    private Button button(String label, boolean primary) {
        ...
        button.setBackgroundColor(primary ? ACCENT : Color.rgb(27, 33, 48));
        return button;
    }
