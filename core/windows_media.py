...
def parse_lrc(text: str) -> list[dict[str, Any]]:
    ...
    for raw in (text or "").splitlines():
        matches = list(_LRC_TIMESTAMP.finditer(raw))
        if not matches:
            continue
...
class SystemMediaBridge:
    """Continuously mirrors Windows system media and accepts remote controls."""
    ...
    async def _drain_commands(self, session):
        while True:
            ...
            try:
                if action == "volume":
                    WindowsAudioSystem.set_volume(float(value))
                    continue
                if action == "mute":
                    WindowsAudioSystem.set_mute(bool(value))
                    continue
                if action == "output":
                    WindowsAudioSystem.set_default_output(str(value))
                    continue
                handled = await self._media_command(session, action, value)
                if not handled:
                    fallback_sent = self._fallback_media_key(action, session)
                    logger.info(
                        "GSMTC command rejected: action=%s fallback_sent=%s",
                        action,
                        fallback_sent,
                    )
            except Exception:
                logger.exception("GSMTC command failed: action=%s", action)
                fallback_sent = self._fallback_media_key(action, session)
                logger.info("Media-key fallback after GSMTC error: action=%s sent=%s", action, fallback_sent)

    @staticmethod
    def _command_accepted(result: Any, action: str) -> bool:
        # Some Python WinRT projections complete RemoteAsync operations with
        # None instead of exposing the documented Boolean result. The command
        # has still been submitted; sending a toggle fallback can undo it.
        if result is None:
            logger.debug("GSMTC RemoteAsync completed without a Boolean result: action=%s", action)
    ...
    async def _media_command(session, action: str, value: Any) -> bool:
        ...
        if action == "play_pause":
            return SystemMediaBridge._command_accepted(await session.try_toggle_play_pause_async(), action)
        if action == "play":
            return SystemMediaBridge._command_accepted(await session.try_play_async(), action)
        if action == "pause":
            return SystemMediaBridge._command_accepted(await session.try_pause_async(), action)
        if action == "stop":
    ...
    @staticmethod
    def _fallback_media_key(action: str, session=None) -> bool:
        if action in ("play", "pause"):
            try:
                status = int(session.get_playback_info().playback_status)
            except Exception:
                logger.warning("Skipped unsafe media-key fallback without playback state: action=%s", action)
                return False
            if action == "play" and status == 4:
                return False
            if action == "pause" and status != 4:
                return False
        keys = {
