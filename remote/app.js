...
let ws = null, state = null, reconnectTimer = null, selectedMacro = null, lyricsKey = "", opened = false;
...
function connect() {
  ...
  socket.onclose = () => {
    ...
    if (!thisOpened && !opened) $("pair").classList.remove("hidden");
...
function fillParams(hostId, mode, detail, specs) {
  ...
  specs.forEach(([key]) => {
    const input=$(`${hostId}-${key}`), value=$(`${hostId}-${key}-value`);
...
function renderLive() {
  const l=state.live||{}, d=l.detail||{}, devices=l.devices||[]; $("live-state").textContent=l.running?"运行中":"停止";
  const selected=devices.find(x=>x.name===l.selected)?.id || devices[0]?.id || "";
  options($("live-device"),devices,selected,"name","id"); options($("live-style"),visualStyles,d.style??"星云",null,null);
...
function renderMedia() {
  ...
  setRange("system-progress",m.position,m.duration); $("system-time").textContent=formatTime(m.position); $("system-duration").textContent=formatTime(m.duration);
  $("system-progress").disabled=!m.can_seek; $("seek-hint").classList.toggle("hidden",!!m.can_seek || !m.available);
  $("media-play").textContent=m.playing?"⏸":"▶"; $("media-play").disabled=!m.available; $("media-prev").disabled=!m.available; $("media-next").disabled=!m.available;
  $("media-repeat").textContent=`循环：${({none:"关",track:"单曲",list:"列表"})[m.repeat]||"关"}`; $("media-repeat").disabled=!controls.repeat;
  $("media-shuffle").textContent=`随机：${m.shuffle?"开":"关"}`; $("media-shuffle").disabled=!controls.shuffle; $("media-stop").disabled=!m.available;
  setRange("system-volume",audio.volume??0); $("system-volume-label").textContent=audio.available?`${Math.round(audio.volume||0)}%`:"不可用"; $("system-volume").disabled=!audio.available; $("system-mute").disabled=!audio.available; $("system-mute").checked=!!audio.mute;
  options($("audio-output"),audio.outputs||[],audio.active_output,"name","id"); $("audio-output").disabled=!audio.available;
...
function render() { if(!state)return; renderStudio(); renderMedia(); renderMacros(); renderSettings(); }

$("pair-form").addEventListener("submit",event=>{event.preventDefault();token=$("pair-pin").value.trim();if(token.length===6){opened=false;localStorage.setItem("launchpadPin",token);connect();}});
...
$("video-playlist").onchange=event=>command("video.select",Number(event.target.value));
$("video-play").onclick=()=>{const v=state?.video||{};command(!v.running||v.paused?"video.play":"video.pause");};
...
$("music-playlist").onchange=event=>command("music.select",Number(event.target.value));
$("music-play").onclick=()=>{const m=state?.music||{};command(!m.running||m.paused?"music.play":"music.pause");};
...
$("live-refresh").onclick=()=>command("live.refresh"); $("live-device").onchange=event=>command("live.device",event.target.value); $("live-style").onchange=event=>command("live.style",event.target.value); $("live-start").onclick=()=>command("live.start"); $("live-stop").onclick=()=>command("live.stop");
$("utility-choice").onchange=event=>command("utility.select",event.target.value); $("utility-start").onclick=()=>command("utility.start"); $("utility-stop").onclick=()=>command("utility.stop"); $("utility-pause").onclick=()=>command("utility.focus_pause"); $("utility-reset").onclick=()=>command("utility.focus_reset");
...
$("media-prev").onclick=()=>command("system_media.previous"); $("media-play").onclick=()=>command("system_media.play_pause"); $("media-next").onclick=()=>command("system_media.next"); $("media-stop").onclick=()=>command("system_media.stop"); bindRange("system-progress","system_media.seek",Number,45);
...
$("forget-pin").onclick=()=>{localStorage.removeItem("launchpadPin");token="";opened=false;if(ws)ws.close();$("pair").classList.remove("hidden");};
connect();
